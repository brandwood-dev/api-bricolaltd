...
export class ExchangeRateService {
  ...
  constructor(
    ...
    if (!this.API_KEY) {
      this.logger.warn(
        '⚠️ EXCHANGE_RATE_API_KEY not found in environment variables',
      );
    }
  }
  ...
  async getExchangeRate(
    ...
    try {
      ...
      if (fromCurrency === toCurrency) {
        this.logger.log(
          `✅ Same currency detected: ${fromCurrency} = ${toCurrency}, rate = 1`,
        );
        return {
          fromCurrency,
          toCurrency,
      ...
      if (cached && Date.now() - cached.timestamp < this.CACHE_DURATION) {
        ...
        this.logger.log(
          `⏰ Cache expired for ${cacheKey} (age: ${Date.now() - cached.timestamp}ms)`,
        );
      } else {
      ...
      // Fetch from API
      this.logger.log(
        `🌐 Fetching rate from API: ${fromCurrency} → ${toCurrency}`,
      );
      ...
      this.logger.error(
        `❌ Error getting exchange rate ${fromCurrency} → ${toCurrency}:`,
        error.message,
      );
      ...
      this.logger.log(
        `🔄 Attempting database fallback for ${fromCurrency} → ${toCurrency}`,
      );
      try {
        ...
        if (dbRate) {
          this.logger.log(
            `✅ Database fallback successful: rate = ${dbRate.rate}`,
          );
          return {
            fromCurrency,
            toCurrency,
  ...
  async getBulkExchangeRates(
    ...
    this.logger.log(
      `📊 Getting bulk exchange rates for base currency: ${baseCurrency}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `✅ Found ${currencies.length} active currencies: ${currencies.map((c) => c.code).join(', ')}`,
      );
      ...
      for (const currency of currencies) {
        if (currency.code !== baseCurrency) {
          this.logger.log(
            `🔄 Getting rate: ${baseCurrency} → ${currency.code}`,
          );
          ...
          this.logger.log(
            `✅ Rate ${baseCurrency} → ${currency.code}: ${exchangeRate.rate}`,
          );
      ...
      this.logger.log(
        `📊 Bulk rates result for ${baseCurrency}:`,
        JSON.stringify(result, null, 2),
      );
      return result;
    } catch (error) {
      this.logger.error(
        `❌ Failed to get bulk exchange rates for ${baseCurrency}:`,
        error.message,
      );
      this.logger.error(`📊 Error stack:`, error.stack);
      throw new HttpException(
  ...
  private async fetchExchangeRateFromAPI(
    ...
    try {
      this.logger.log(
        `Attempting to fetch exchange rate from Pro API: ${fromCurrency} -> ${toCurrency}`,
      );
      ...
      if (
        ...
        throw new Error(
          `Exchange rate not found for ${fromCurrency} to ${toCurrency} in Pro API response`,
        );
      }

      this.logger.log(
        `✅ Successfully fetched rate from Pro API: ${fromCurrency} -> ${toCurrency} = ${response.data.conversion_rates[toCurrency]}`,
      );
      ...
      if (!preventRecursion && fromCurrency !== 'USD' && toCurrency !== 'USD') {
        try {
          this.logger.log(
            `Trying fallback via USD: ${fromCurrency} -> USD -> ${toCurrency}`,
          );
          // Prevent infinite recursion by setting preventRecursion to true
          const usdToFrom = await this.fetchExchangeRateFromAPI(
            'USD',
            fromCurrency,
            true,
          );
          const usdToTo = await this.fetchExchangeRateFromAPI(
            'USD',
            toCurrency,
            true,
          );
          ...
          this.logger.log(
            `Successfully calculated rate via USD: ${fromCurrency} -> ${toCurrency} = ${calculatedRate}`,
          );
        ...
        this.logger.log(
          `Skipping USD fallback to prevent recursion: ${fromCurrency} -> ${toCurrency}`,
        );
      }
      ...
      try {
        this.logger.log(
          `Trying database fallback for: ${fromCurrency} -> ${toCurrency}`,
        );
        ...
        if (existingRate) {
          this.logger.log(
            `Found existing rate in database: ${fromCurrency} -> ${toCurrency} = ${existingRate.rate}`,
          );
        ...
        if (inverseRate) {
          ...
          this.logger.log(
            `Found inverse rate in database: ${toCurrency} -> ${fromCurrency} = ${inverseRate.rate}, calculated: ${calculatedRate}`,
          );
          return calculatedRate;
        }
      } catch (dbError) {
      ...
      if (defaultRate) {
        this.logger.warn(
          `Using hardcoded default rate: ${fromCurrency} -> ${toCurrency} = ${defaultRate}`,
        );
        return defaultRate;
      }
  ...
  async updateExchangeRates(): Promise<void> {
    ...
    try {
        ...
        for (const toCurrency of currencyCodes) {
          if (fromCurrency !== toCurrency) {
            try {
              ...
              this.logger.warn(
                `Failed to update rate ${fromCurrency} to ${toCurrency}`,
                error,
              );
            }
          }
      ...
      this.logger.error(
        'Failed to complete scheduled exchange rate update',
        error,
      );
    }
  }

...
export class UserSessionController {
  ...
  @ApiResponse({ status: 201, description: 'User session created successfully.', type: UserSession })
  ...
  @ApiResponse({ status: 200, description: 'List of user sessions.', type: [UserSession] })
  ...
  @ApiResponse({ status: 200, description: 'Current session details.', type: UserSession })
  ...
  @ApiResponse({ status: 200, description: 'User session details.', type: UserSession })
  ...
  @ApiResponse({ status: 200, description: 'User session updated successfully.', type: UserSession })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required.' })
  @ApiResponse({ status: 404, description: 'User session not found.' })
  ...
  @ApiResponse({ status: 200, description: 'Session activity updated.', type: UserSession })
  @ApiResponse({ status: 404, description: 'User session not found.' })
  async updateActivity(@Param('id') id: string, @Request() req) {
    ...
    if (session.userId !== req.user.id) {
      throw new ForbiddenException('You can only update your own session activity');
    }
  ...
  async revokeAllSessions(@Request() req) {
    ...
    await this.userSessionService.revokeAllUserSessions(req.user.id, currentSessionId);
    return { message: 'All other sessions revoked successfully' };
  }

  @Get('user/:userId')
  @UseGuards(AdminGuard)
  @ApiOperation({ summary: 'Get all sessions for a specific user (Admin only)' })
  @ApiResponse({ status: 200, description: 'List of user sessions.', type: [UserSession] })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required.' })
  ...
  @ApiResponse({ status: 200, description: 'User session deleted successfully.' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required.' })
  @ApiResponse({ status: 404, description: 'User session not found.' })

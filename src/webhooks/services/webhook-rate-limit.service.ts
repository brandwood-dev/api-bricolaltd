...
export class WebhookRateLimitService {
  ...
  async isWithinRateLimits(
    ...
    try {
      ...
      const ipResult = this.checkRateLimit(`ip:${ipAddress}`, this.limits.perIp);
      if (!ipResult.allowed) {
      ...
      if (eventType) {
        const eventResult = this.checkRateLimit(`event:${eventType}`, this.limits.perEvent);
        if (!eventResult.allowed) {
          return { allowed: false, reason: `Rate limit exceeded for event type: ${eventType}` };
        }
      }
   ...
   */
  private checkRateLimit(key: string, limit: { maxRequests: number; windowMs: number }): { allowed: boolean; remaining?: number; resetTime?: number } {
    const now = Date.now();
    ...
    if (!entry || now >= entry.resetTime) {
      ...
      return { allowed: true, remaining: limit.maxRequests - 1, resetTime: newEntry.resetTime };
    }
   ...
   */
  getRateLimitStatus(ipAddress: string, eventType?: string): Record<string, any> {
    try {
      const now = Date.now();
      ...
      status.global = {
        ...
        remaining: globalEntry ? Math.max(0, this.limits.global.maxRequests - globalEntry.count) : this.limits.global.maxRequests,
      ...
      status.ip = {
        ...
        remaining: ipEntry ? Math.max(0, this.limits.perIp.maxRequests - ipEntry.count) : this.limits.perIp.maxRequests,
      ...
      if (eventType) {
        ...
        status.event = {
          ...
          remaining: eventEntry ? Math.max(0, this.limits.perEvent.maxRequests - eventEntry.count) : this.limits.perEvent.maxRequests,

...
export class PaymentTransactionController {
  ...
  @ApiResponse({
    status: 404,
    description: 'Transaction or provider not found.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Return payment transactions by status.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Return payment transactions by provider.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  findByProviderId(@Param('providerId') providerId: string) {
    return this.paymentTransactionService.findByProviderId(
      parseInt(providerId),
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Return payment transactions by transaction.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Get payment transaction by provider transaction ID',
  })
  ...
  findByProviderTransactionId(
    ...
    return this.paymentTransactionService.findByProviderTransactionId(
  ...
  @ApiResponse({
    status: 200,
    description:
      'The payment transaction status has been successfully updated.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  @ApiResponse({ status: 404, description: 'Not found.' })

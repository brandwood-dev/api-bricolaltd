...
export class ThreeDSecureController {
  ...
  async initialize3DSecure(
    ...
    try {
      ...
      if (!result.success) {
        throw new BadRequestException(result.error || '3D Secure initialization failed');
      }
      ...
      return {
        ...
        authenticationFlow: result.requiresAction ? 'challenge' : 'frictionless',
      };

    } catch (error) {
      this.logger.error(`Failed to initialize 3D Secure for user ${req.user.id}:`, error);
      
      if (error instanceof BadRequestException) {
        throw error;
      }

      throw new InternalServerErrorException('Failed to initialize 3D Secure authentication');
    }
  }
  ...
  async completeChallenge(
    ...
    try {
      this.logger.log(`Completing 3D Secure challenge for user: ${req.user.id}`, {
      ...
      if (!result.success) {
        throw new BadRequestException(result.error || '3D Secure challenge failed');
      }
      ...
      this.logger.error(`Failed to complete 3D Secure challenge for user ${req.user.id}:`, error);
      
      if (error instanceof BadRequestException) {
        throw error;
      }

      throw new InternalServerErrorException('Failed to complete 3D Secure challenge');
    }
  }
  ...
  async check3DSecureRequired(
    ...
    try {
      const { amount, currency = 'gbp', cardCountry, issuingCountry } = req.query;
      ...
      throw new InternalServerErrorException('Failed to check 3D Secure requirements');
    }
  }
  ...
  async get3DSecureStats(
    ...
    try {
      ...
      throw new InternalServerErrorException('Failed to get 3D Secure statistics');
    }
  }
  ...
  async test3DSecure(
    ...
    try {
      ...
      const testResult = await this.threeDSecureService.is3DSecureRequired(100, 'gbp', 'GB', 'GB');

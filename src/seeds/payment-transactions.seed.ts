...
export async function seedPaymentTransactions(dataSource: DataSource) {
  ...
  const paymentTransactionRepository = dataSource.getRepository(PaymentTransaction);
  ...
  if (providers.length === 0 || transactions.length === 0) {
    console.log('⚠️ No providers or transactions found, skipping payment transactions seeding');
    return;
  }
  ...
  for (let i = 0; i < Math.min(80, transactions.length); i++) {
    ...
    switch (paymentMethod) {
      case PaymentMethod.CARD:
        ...
        providerStatus = status === 'completed' ? 'succeeded' : status === 'pending' ? 'processing' : 'failed';
        break;
      case PaymentMethod.PAYPAL:
        providerTransactionId = `PAYID-${faker.string.alphanumeric(10).toUpperCase()}`;
        providerStatus = status === 'completed' ? 'COMPLETED' : status === 'pending' ? 'PENDING' : 'FAILED';
        break;
    ...
    const paymentTransaction = paymentTransactionRepository.create({
      ...
      providerStatus,
      processedAt: status === 'completed' ? faker.date.recent({ days: 30 }) : undefined,
      transaction,

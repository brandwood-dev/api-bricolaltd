...
export async function seedUsers(dataSource: DataSource) {
  ...
  try {
    ...
    if (!france) {
      console.error(
        '❌ France country not found in database. Make sure countries are seeded first.',
      );
    ...
    if (!belgium) {
      console.error(
        '❌ Belgium country not found in database. Make sure countries are seeded first.',
      );
      throw new Error('Required country (BE) not found');
    }

    console.log(
      `✓ Found countries: France (${france.id}), Belgium (${belgium.id})`,
    );
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const email = faker.internet
        .email({
          firstName,
          lastName,
          provider: isFromFrance ? 'email.fr' : 'email.be',
        })
        .toLowerCase();
    ...
    for (const userData of usersData) {
      ...
      if (!existingUser) {
        console.log(
          `Creating user: ${userData.email} with country ID: ${userData.countryId}`,
        );

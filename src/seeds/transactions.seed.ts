...
export async function seedTransactions(dataSource: DataSource) {
  ...
  if (users.length < 2 || wallets.length === 0) {
    console.log('⚠️ Insufficient users or wallets found, skipping transactions seeding');
    return;
  }
  ...
  for (let i = 0; i < transactionsData.length; i++) {
    ...
    const transaction = transactionRepository.create({
      ...
      processedAt: transactionData.status === TransactionStatus.COMPLETED ? new Date() : undefined,
    });

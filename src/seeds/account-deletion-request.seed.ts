...
export async function seedAccountDeletionRequests(dataSource: DataSource) {
  ...
  const deletionRequestRepository = dataSource.getRepository(
  ...
  if (users.length === 0) {
    console.log(
      '⚠️ No users found, skipping account deletion requests seeding',
    );
    return;
  }
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const admin =
      admins.length > 0 ? faker.helpers.arrayElement(admins) : undefined;
    ...
    if (!existingRequest) {
      ...
      const deletionRequest = deletionRequestRepository.create({
        ...
        reviewedByAdminId:
          status !== DeletionStatus.PENDING ? admin?.id : undefined,
        reviewedAt,
      });

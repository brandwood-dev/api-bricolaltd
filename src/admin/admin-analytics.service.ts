...
export class AdminAnalyticsService {
  ...
  async getBookingAnalytics(period: string) {
    ...
    const [bookingTrends, statusDistribution, averageBookingValue] =
      await Promise.all([
  ...
  async getPerformanceMetrics(period: string) {
    ...
    const [conversionRate, averageSessionDuration, bounceRate] =
      await Promise.all([
  ...
  async getFormattedAnalyticsData(period: string = '30d') {
    console.log(
      '🔍 [DATABASE] Fetching formatted analytics data from database...',
    );
    ...
    try {
      ...
      const [totalUsers, totalBookings, totalTools, currentRevenue] =
        await Promise.all([
      ...
      const [previousUsers, previousBookings, previousRevenue] =
        await Promise.all([
          this.userRepository.count({
      ...
      const userGrowth =
        previousUsers > 0
          ? ((currentUserCount - previousUsers) / previousUsers) * 100
          : 0;
      const revenueGrowth =
      ...
      const formattedData = {
        ...
        charts: {
          ...
          categories: (toolData.categoryPerformance || []).map((item: any) => {
            const totalBookings = toolData.categoryPerformance.reduce(
              (sum: number, cat: any) => sum + (cat.bookings || 0),
              0,
            );
            return {
              ...
              percentage:
                totalBookings > 0
                  ? Math.round(
                      ((item.bookings || 0) / totalBookings) * 100 * 100,
                    ) / 100
                  : 0,
            };
          }),
      ...
      console.error(
        '❌ [DATABASE] Error fetching formatted analytics data:',
        error,
      );
      throw error;
    }
